			...
			<div class="navbar-nav ml-auto mx-auto ">
				 ...
				 <a class="nav-item nav-link" href="<c:url value="/main/tv_show.htm"/>">TV SHOWS</a> <a
					...
					<!-- Dropdown theo tên người dùng để update -->
							...
							${message}
							<a href="<c:url value="/main/insert_movie.htm"/>"><button type="button" class="btn btn-primary btn-insert-movie">Insert</button></a>
								<c:forEach var="m" items="${movies}">
									<div class="media">
									  <a href="<c:url value="/main/detail_page/${m.movie_id}.htm"/>"><img src="${m.movie_avatar_image}" class="mr-3" alt="avatar"></a>
									  <div class="media-body">
									    <h5 class="mt-0">${m.movie_name}</h5>
									    ${m.category.category_name}
									    ${m.genre.genre_name}
									    ${m.introduction}
									    <a href="<c:url value="/main/update_movie/${m.movie_id}.htm"/>"><button type="button" class="btn btn-primary">Update</button></a>
									    	<a href="<c:url value="/main/delete_movie/${m.movie_id}.htm"/>"><button type="button" class="btn btn-primary">Delete</button></a>
